...
export const RedisCache = (): Cache => {
  ...
  return {
    ...
    async get<T extends Value>(key: Key): Promise<T> {
      ...
      try {
        finalResult = await redis.hgetall(key)
        ...
        if (finalResult.type === 'number') {
          finalResult = Number(finalResult.data) as T // we need to convert the stringified number back to a number
        } else if (finalResult.type === 'string') {
          finalResult = finalResult.data as T // no conversion needed, ioredis stores all them values as strings
        } else {
          finalResult = JSON.parse(finalResult.data) as T // convert the json string back to an object
        }
      } catch (e) {
